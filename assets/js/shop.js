fetch(
  ...
  .then((myData) => {
    ...
    product_shop.querySelector(
      ".set-bg"
    ).style.backgroundImage = `url(${shop_info.image_profile})`;
    product_shop.querySelector(".shop__image-name").innerText =
      shop_info.user_nickName;
    product_shop.querySelector(".sold-count").innerText = shop_info.sold_amount;
    product_shop.querySelector(".customer-count").innerText =
      shop_info.sold_amount;
    product_shop.querySelector(".product-count").innerText =
      currentProductArr.length;
    ...
    if (clone_shop_products.length <= 6) {
      divided =
        clone_shop_products.length != 5
          ? 12 / clone_shop_products.length + ""
          : "2-4";
    } else {
