...
const logInMenuOnMobile = query(
  ".navbar-list__item.navbar-user.hide.hide-special"
);
...
window.onload = (e) => {
  if (localStorage.getItem("login") == "success") {
    ...
    fetch(
      "https://otaku-space-server.vercel.app/userSearchHistory/" +
    ...
    fetch(
      "https://otaku-space-server.vercel.app/userCart/" +
        localStorage.getItem("userid")
    )
      ...
      .then((data) => {
        ...
        if (data.length != 0) {
          query(".header__cart-list").classList.remove(
            "header__cart-list--no-cart"
          );
        }
        var myCartUL = query(".cart__item-list");
        myCartUL.innerHTML = data.reduce((a, b) => {
          var priceAfterSelled =
            (parseInt(b.price) * (1 - b.sale_percent / 100)) / 1000;
          priceAfterSelled = priceAfterSelled.toFixed() * 1000;
          var cartEle = `<li class="cart__item" id = "${b.cart_user_id}-${
            b.product_id
          }">
                ...
                <div class="cart__item--info">
                    <div class="cart__item-head">
                        ...
                        <div class="cart__item-price-wrap">
                            <span class="cart__item-price">${numberWithCommas(
                              priceAfterSelled
                            )}đ</span>
                    ...
                    <div class="cart__item-body">
                        <span class="cart__item-descript">
                            Phân loại: <span class="product-category">${
                              b.category
                            }</span>
                        </span>
    ...
    fetch(`https://otaku-space-server.vercel.app/randomProduct/5`)
      .then((res) => res.json())
      .then((data) => {
        data = data.data;
        searchResult.innerHTML = data.reduce((a, b, c) => {
  ...
  if (window.location.href.includes("nologin.html")) {
    fetch("https://otaku-space-server.vercel.app/allProduct/")
      .then((response) => response.json())
      .then((data) => {
        ...
        productNLArray = queryAll(
          ".home-product.wrapper-here .row .col.l-2-4.m-4.c-6"
        );
        // productType=
        return "Done !!";
      })
  ...
  if (window.location.href.includes("productPage.html")) {
    // Take Random Product
    fetch("https://otaku-space-server.vercel.app/randomProduct/6")
      .then((response) => response.json())
      .then((data) => {
    ...
    // Take product info
    fetch(
      "https://otaku-space-server.vercel.app/productID/" +
      ...
      .then((data) => {
        ...
        var newPrice =
          (productData.price * (1 - parseInt(salePercent) / 100)) / 1000;
        // see(productData);
        // var produdata.data[0])
        query(
          ".shop__image-logo.set-bg"
        ).style.backgroundImage = `url(${productData.image_profile})`;
        ...
        query(".original-pr").textContent =
          numberWithCommas(productData.price) + "đ";
        query(".after-selled").textContent = numberWithCommas(
          newPrice.toFixed() * 1000
        );
        query(".sale-percent").textContent = salePercent + "% giảm";
        query(".product-category--special").textContent = productData.category;
        query(
          ".image-info--main.set-bg"
        ).style.backgroundImage = `url(${productData.main_image})`;
        for (var i = 1; i <= 5; i++) {
          if (i == 1) {
            query(
              `.set-bg.info-sub-1`
...
searchInput.onkeyup = (e) => {
  ...
  if (text) {
    ...
    fetch(`https://otaku-space-server.vercel.app/searchProduct/${text}`)
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(`https://otaku-space-server.vercel.app/searchProduct/t`)
      .then((res) => res.json())
      .then((data) => {
        query(".history-heading").textContent = "Kết quả tìm kiếm";
        searchResult.innerHTML = data.reduce((a, b, c) => {
...
searchInput.onfocus = (e) => {
  if (searchInput.value == "" && localStorage.getItem("login") == "success") {
    ...
    // fetch('https://otaku-space-server.vercel.app/searchHistory')
...
cartList.onclick = (e) => {
  ...
  if (delBtn.classList.contains("cart__item-remove")) {
    ...
    see(deleteUserID);
    see(deleteProductID);
    fetch("https://otaku-space-server.vercel.app/deleteCartItem", {
      headers: {
...
const toast_message = ({
  ...
  msg = "",
  duration = 3000,
  icon = `<i class="fa-sharp fa-solid fa-shield-check"></i>`,
}) => {
...
function redirectToProductPage(product) {
  ...
  var salePercent = product
    .querySelector(".home-product-item__sale-off-percent")
    .textContent.split("%")[0];
  localStorage.setItem("productSalePercent", salePercent);
  window.location.href = "/productPage.html";
}

function redirectToProductPageAfterPostProduct(
  productID,
  sale,
  userid,
  productName,
  ...
  if (userid == 100 && localStorage.getItem("login") == "success") {
    ...
    fetch(
      `https://otaku-space-server.vercel.app/saveSearch/${text}/${localStorage.getItem(
        "userid"
      )}`
    )
      .then((res) => res.json())
...
function getProductHTML(productInfo, sign) {
  ...
  var afterCellPrice =
    (parseInt(productInfo.price) * (1 - salePercent / 100)) / 1000;
  var myHTML = `<div class="col l-${sign} m-4 c-6" productType="${
    ...
    <a class="home-product-item" href="./productPage.html" onclick="event.preventDefault();redirectToProductPage(event.target.parentElement.parentElement)">
        ...
        <div class="home-product-item__price">
            <span class="home-prodct-item__price--old">${numberWithCommas(
              productInfo.price
            )}đ</span>
            <span class="home-prodct-item__price--new">${numberWithCommas(
              afterCellPrice.toFixed() * 1000
            )}đ</span>
        </div>
...
function getBoxProduct(productInfo, ratio) {
  ...
  var afterCellPrice =
    (parseInt(productInfo.price) * (1 - salePercent / 100)) / 1000;
  var myHTML = `<div class="col l-${ratio} m-4 c-6 box_product"  productType="${
    productInfo.category
  }">
    <div class="box_product_shop">
        <div class = "${productInfo.product_id} ${
