document.addEventListener("DOMContentLoaded", function () {
  Validator({
    ...
    onSubmit: function (data) {
      fetch("https://otaku-space-server.vercel.app/signup", {
        headers: {
        ...
        .then((myData) => {
          if (myData.exist == true) {
            ...
            inputEle.querySelector(".form-message").textContent =
  ...
  Validator({
    ...
    rules: [
      Validator.isUserName(
        "#usernameLogIn",
        "Vui lòng nhập tên đăng nhập đầu đủ"
      ),
      Validator.isRequired("#passwordLogIn", "Vui lòng nhập mật khẩu"),
    ],
    onSubmit: function (data) {
      fetch("https://otaku-space-server.vercel.app/signin", {
        headers: {
          "Content-type": "application/json",
        },
        method: "POST",
  ...
  Validator({
    ...
    rules: [
      ...
      Validator.isSalePercent(
        "#productSale",
        100,
        "Giá trị phải trong khoảng 0-100"
      ),
    ...
    onSubmit: function (data) {
      ...
      fetch("https://otaku-space-server.vercel.app/new-product", {
        headers: {
          "Content-type": "application/json",
        },
        method: "POST",
        ...
        .then((dataBackEnd) => {
          ...
          redirectToProductPageAfterPostProduct(
            info.id,
            info.sale_percent,
  ...
  return {
    ...
    test: function (value) {
      ...
      return re.test(value)
        ? undefined
        : message || "Vui lòng chèn link ảnh đúng định dạng";
    },
  };
};
...
Validator.isSalePercent = (selector, max, message) => {
  return {
    selector: selector,
    test: function (value) {
      return parseInt(value) <= max && parseInt(value) >= 0
        ? undefined
        : message || "Giá trị không họp lệ";
    },
  };
};
