...
fetch(
  `https://otaku-space-server.vercel.app/user_shop/${localStorage.getItem(
    "shop_name"
  )}`
)
...
heartButton.onclick = (k) => {
  if (localStorage.getItem("login") != "success") {
    ...
    fetch(
      `https://otaku-space-server.vercel.app/updateProductLike/${localStorage.getItem(
        "productID"
      )}/${like}`
    )
      .then((response) => response.json())
...
addToCartBtn.onclick = (e) => {
  if (localStorage.getItem("login") != "success") {
    ...
    itemFigure = 0;
    if (productCount != 0) {
      var checkProductInCart = query(
        `[id="${localStorage.getItem("userid")}-${localStorage.getItem(
          "productID"
        )}"]`
      );
      if (!checkProductInCart) {
        ...
        query(".cart-notice").textContent =
          parseInt(query(".cart-notice").textContent) + 1;
        query(
          ".cart__item-list"
        ).innerHTML += `<li class="cart__item" id = "${localStorage.getItem(
          "userid"
        )}-${localStorage.getItem("productID")}"> 
                    <img src="${query(".image-info--main.set-bg")
                      .style.backgroundImage.slice(4, -1)
                      .replace(/"/g, "")}" class="cart__item--img">
                    <div class="cart__item--info">
                        <div class="cart__item-head">
                            <h5 class="cart__item-name text-left long-name">${query(
                              ".col.l-12.product-title"
                            ).textContent.trim()}</h5>
                            <div class="cart__item-price-wrap">
                                <span class="cart__item-price">${
                                  query(".after-selled").textContent
                                }đ</span>
                        ...
                        <div class="cart__item-body">
                            <span class="cart__item-descript">
                                Phân loại: <span class="product-category">${
                                  query(".product-category--special")
                                    .textContent
                                }</span>
                            </span>
        ...
        fetch(
          `https://otaku-space-server.vercel.app/addToCart/${localStorage.getItem(
            "productID"
          )}/${localStorage.getItem("userid")}/${productCount}`
        )
          .then((response) => response.json())
          .then((data) => see(data));
      } else {
        var myCSS = `[id="${localStorage.getItem(
          "userid"
        )}-${localStorage.getItem("productID")}"]`;
        var myProductInCart = query(myCSS);
        var myProductInCartCount = myProductInCart.querySelector(
          ".cart__item-quantity"
        );
        if (
          parseInt(myProductInCartCount.textContent) + productCount >
          parseInt(query(".data-count").textContent)
        ) {
          toast_message({
            type: "loginPls",
            duration: 1000,
            msg: "Vượt quá số lượng sản phẩm đang có !",
            icon: '<i class="fa-solid fa-triangle-exclamation"></i>',
          });
        } else {
          toast_message({
          ...
          myProductInCartCount.textContent =
            parseInt(myProductInCartCount.textContent) + productCount;
          fetch(
            `https://otaku-space-server.vercel.app/updateCart/${localStorage.getItem(
              "productID"
            )}/${localStorage.getItem("userid")}/${productCount}`
          )
            .then((response) => response.json())
