...
$(document).ready(function () {
  fetch("https://otaku-space-server.vercel.app/getCategory")
    .then((res) => res.json())
    .then((data) => {
      see(data);
      query(
        ".category-list"
      ).innerHTML = `<li class="category-item category-item--active">
                                            <a href="#" class="category-item__link">Tất Cả</a>
                                            </li>`;
      query(
        ".mobile-category"
      ).innerHTML = `<li class="mobile-category__item mobile-category__item-selected">
      ...
      data.forEach((e) => {
        ...
        query(
          ".mobile-category"
        ).innerHTML += `<li class="mobile-category__item">
...
function initEvents() {
  ...
  queryAll(".mobile-category__item").forEach(function (item) {
    item.onclick = (e) => {
      e.preventDefault();
      $(".mobile-category__item-selected").removeClass(
        "mobile-category__item-selected"
      );
  ...
  homeBtn.forEach((e) => {
    e.onclick = () => {
      const productNLArrayNew = queryAll(
        ".home-product .row .col.l-2-4.m-4.c-6"
      );
      const productArray = Array.prototype.slice.call(productNLArrayNew);
  ...
  function extractPrice(product) {
    return turnMoneyStringToNumber(
      product
        .querySelector(".home-prodct-item__price--new")
  ...
  function sortHighLow(a, b) {
    return (
      (b.price * (100 - b.sale_percent)) / 100 -
      (a.price * (100 - a.sale_percent)) / 100
    );
  }
