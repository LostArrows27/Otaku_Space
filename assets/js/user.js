// Const for use later
const menuComponent = Array.prototype.slice.call(
  queryAll(".user__interact--heading")
);
const contentComponent = Array.prototype.slice.call(
  queryAll(".user__information")
);
...
fetch(
  "https://otaku-space-server.vercel.app/userOrder/" +
    localStorage.getItem("userid")
)
  .then((res) => res.json())
  .then((data) => {
    data.reverse();
    query(".col.l-11.user__info-text.scroll_order").innerHTML = data.reduce(
      (a, b) => {
    ...
    queryAll(".test").forEach((e) => {
      e.onclick = (f) => {
        ...
        query(".order-heading").textContent = `Đơn thanh toán mã ${
          e.querySelector(".order-id").textContent
        }`;
        query(".orders-overAll").textContent = `${
          e.querySelector(".order-money").textContent
        }`;
        query(".order-ul").innerHTML = data
          .filter((a) => {
            return (
              a.orderid == parseInt(e.querySelector(".order-id").textContent)
            );
          })[0]
          .productInfo.reduce((a, b) => {
            var myHTMLLI = `<li class="cart__item">
                    <img src="${b.img}" alt=""
                        class="cart__item--img--special">
                    <div class="cart__item--info">
                        <div class="cart__item-head">
                            <h5 class="cart__item-name--special long-name">${
                              b.productName
                            }</h5>
                        ...
                        <div class="cart__item-body">
                            <span class="cart__item-descript">
                                Phân loại: <span class="product-category--special purchased-box">${
                                  b.category
                                }</span>
                            </span>
                        </div>
